...
class BaseDetector(nn.Module):
    ...
    @property
    def with_temp(self):
        return hasattr(self, 'temp_head') and self.temp_head is not None
    ...
    def show_result(self, 
        ...
        for img, img_meta in zip(imgs, img_metas):
            ...
            # cur_save_dir = '../Results/perobj/%s/%s' % (img_meta['video_name'], img_meta['frame_name'])
            cur_save_dir = '%s/%s/%s' % (save_dir_perobj, img_meta['video_name'], img_meta['frame_name'])
            ...
            if segm_result is not None:
                ...
                for i in inds:
                    ...
                    if mask.shape[0] != h or mask.shape[1] != w:
                        new_mask = np.zeros((h,w),dtype=np.bool)
                        new_mask[:mask.shape[0],:mask.shape[1]] = mask
                        mask = new_mask
                    assert mask.shape == (h,w), (mask.shape, h, w)
                    mask = mask[:h, :w]
                    cur_save_path = osp.join(cur_save_dir, '%d.png' % i)
                    cv2.imwrite(cur_save_path, mask*255)
                    if not is_video:
                      color_id = i
                    else:
                      color_id = obj_ids[i]
            ...
            if save_vis:
              ...
              save_path = '{}/{}/{}.png'.format(save_path, img_meta['video_name'], img_meta['frame_name'])
            else:
              show = True
              save_path = None
            # draw bounding boxes
