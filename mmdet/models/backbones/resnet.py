...
from ..registry import BACKBONES, BACKBONES_MEM
...
class ResNetMem(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1_p = nn.Conv2d(1, 64, kernel_size=7, padding=3, stride=2)
    ...
    def forward(self, x, l, n):
        ...
        outs = []
        '''add l'''
        l = F.interpolate(l[:,None], size=(x.shape[2]*2, x.shape[3]*2), mode='bilinear', align_corners=True)
        new_x = []
        for i in range(len(n)):
            if n[i] > 0:
                cur_x = x[i:i+1].expand(n[i],-1,-1,-1)
                new_x.append(cur_x)
        x = torch.cat(new_x, 0)
        x = x + self.conv1_p(l)
        ''''''
    ...
    def train(self, mode=True):
        super(ResNetMem, self).train(mode)
        if mode and self.norm_eval:
            for m in self.modules():
                # trick: eval have effect on BatchNorm only
                if isinstance(m, nn.BatchNorm2d):
