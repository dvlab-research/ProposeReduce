...
class FCNMaskHead(nn.Module):
    ...
    def get_target_ext(self, bboxes, sampling_results, gt_masks, rcnn_train_cfg):
        pos_assigned_gt_inds = [
            res.pos_assigned_gt_inds for res in sampling_results
        ]
        mask_targets = mask_target(bboxes, pos_assigned_gt_inds,
                                   gt_masks, rcnn_train_cfg)
        return mask_targets

    def get_target_ext_test(self, bboxes, masks, rcnn_train_cfg):
        gt_inds = [torch.arange(m.shape[0]).cuda() for m in bboxes]
        mask_targets = mask_target(bboxes, gt_inds,
                                   masks, rcnn_train_cfg)
        return mask_targets
